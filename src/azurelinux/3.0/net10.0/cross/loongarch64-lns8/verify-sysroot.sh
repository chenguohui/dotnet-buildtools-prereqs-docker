...
# verify-sysroot.sh
#
# Runs inside the lns8 cross image to verify sysroot completeness:
...
# Highest GLIBC_2.x symbol version the target lns8 system (glibc 2.28) provides.
# Safe under `set -u`; can be overridden from the environment.
GLIBC_EXPECTED="${GLIBC_EXPECTED:-2.28}"
...
echo "--- 动态链接器 ---"
# Old-world ABI systems (lns8) use /lib64/ld.so.1 (new world uses
# ld-linux-loongarch-lp64d.so.1).
find "${ROOTFS_DIR}" -name "ld.so.1" \( -type f -o -type l \) 2>/dev/null | head -5 || echo "  ❌ 未找到"
echo ""
...
if clang --target="$DETECTED_TRIPLE" --sysroot="${ROOTFS_DIR}" -fuse-ld=lld \
    -mcmodel=large -o /tmp/test /tmp/test.c 2>&1; then
    echo "  ✅ 编译成功"
    echo "  --- ELF 结构 ---"
    ...
    if [[ -n "$GLIBC_EXPECTED" && -n "$FLOOR_VER" && "$FLOOR_VER" > "$GLIBC_EXPECTED" ]]; then
        ...
        echo "  ✅ floor ≤ 目标 glibc $GLIBC_EXPECTED，产物可在 lns8 上运行"
    fi
else
    echo "  ❌ 编译失败"
    echo "  --- 诊断信息 ---"
    ...
    clang --target="$DETECTED_TRIPLE" --sysroot="${ROOTFS_DIR}" -fuse-ld=lld \
        -mcmodel=large -o /tmp/test /tmp/test.c -v 2>&1 | grep -E 'search starts|Selected GCC|programs' || true
    echo "  crt 文件:"
